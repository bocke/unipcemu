...
void Tseng4k_status_startXYblock(byte is_screentoscreen, byte doResume) //Starting an X/Y block!
{
	if (doResume) //Perform resume and become an active accelerator by this?
	{
		...
		et34k(getActiveVGA())->W32_acceleratorwassuspended = 0; //We weren't suspended!
	}
...
{
	byte result;
	if ((result = et34k(getActiveVGA())->W32_MMUqueuefilled)!=0) //The queue was filled?
	{
...
byte Tseng4k_writeMMUaccelerator(byte area, uint_32 address, byte value)
{
	...
	if (et34k(getActiveVGA())->W32_MMUqueuefilled||et34k(getActiveVGA())->W32_MMUsuspendterminatefilled) //Queue already filled?
	{
	handleQueueWaiting:
	...
	if (area != 4) //Not to the MMU queue?
	{
		...
		{
			if (et34k(getActiveVGA())->W32_ACLregs.ACL_active) //Not starting a new operation?
			{
...
{
	if ((et34k(getActiveVGA()))->W32_MMUqueuefilled == 4) //Queue filled for the ACL registers?
	{
